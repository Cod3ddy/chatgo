...
// func New(w io.Writer, minLevel slog.Level, serviceName string, traceIDFn TraceIDFn) *Logger{
...
func NewWithEvents(w io.Writer, minLevel slog.Level, serviceName string, traceIDFn TraceIDFn, events Events) *Logger {
	return new(w, minLevel, serviceName, traceIDFn, events)
}
...
func new(w io.Writer, minLevel slog.Level, serviceName string, traceIDFn TraceIDFn, events Events) *Logger {
	// Convert the file name to just name.ext when this key/value will
	// be logged
	...
	// Construct the slow JSON handler for use.
